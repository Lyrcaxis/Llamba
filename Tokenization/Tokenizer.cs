...
namespace Llamba.Tokenization {
    ...
    public class Tokenizer {
        ...
        static Tokenizer() {
            if (Model.instance.format is LLama3Format) { InitializeTokenizer("Tokenization/llama-3-tokenizer.json"); }
            else if (Model.instance.format is Gemma2Format) { InitializeTokenizer("Tokenization/gemma-2-tokenizer.json"); }
            else if (Model.instance.format is MistralFormat) {
                if (Model.vocab.Count > 130000) { InitializeTokenizer("Tokenization/mistral-nemo-tokenizer.json"); }
                else { InitializeTokenizer("Tokenization/mistral-tokenizer.json"); }
            }
        }
        ...
        public static void InitializeTokenizer(string tokenizerPath) {
            ...
            // Make sure the python application exits together with the server.
