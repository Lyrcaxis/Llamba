		...
		void ISampler.Initialize(ChatQuery query, IEnumerable<int> promptTokens) {
			...
			if (repetition_penalty != 1f || presence_penalty != 0f || frequency_penalty != 1f) {
				...
				for (int i = samplerParams.promptTokens.Count - 1; i > lowerPenalizableTokenIndex; i--) {
					if (repetition_penalty != 1f) { _samplerParams.distinctTokenList.Add(samplerParams.promptTokens[i]); }
					if (frequency_penalty != 0f) { _samplerParams.frequencyPenaltyBuffer[samplerParams.promptTokens[i]] -= frequency_penalty; }
					if (presence_penalty != 0f) { _samplerParams.presencePenaltyBuffer[samplerParams.promptTokens[i]] -= presence_penalty; }
				}
			}
		}
